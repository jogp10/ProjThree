...
class MyApp  {
    ...
    initCameras() {
        ...
        perspective1.position.set(0,200,10)
        ...
        this.cameras['3rdPerson'] = camera

        // Create a camera for the parking lot perspective
        const parkingLot = new THREE.PerspectiveCamera( 75, aspect, 0.1, 1000 )
        parkingLot.position.set(4.5,-18.4,0)
        parkingLot.lookAt( new THREE.Vector3(0,-20,0) );
        this.cameras['Parking Lot'] = parkingLot

        // Fixed camera for the menu

        const menuCamera = new THREE.OrthographicCamera( left, right, top, bottom, near, far);
        menuCamera.up = new THREE.Vector3(0,1,0);
        menuCamera.position.set(0,-30, this.frustumSize /4)

        this.cameras['Menu'] = menuCamera

        const chooseObjectCamera = new THREE.PerspectiveCamera( 75, aspect, 0.1, 1000 )
        chooseObjectCamera.position.set(4.5,-18.4,0)

        chooseObjectCamera.lookAt( new THREE.Vector3(0,-20,0) );


        this.cameras['Choose Object'] = chooseObjectCamera

        const placeObjectCamera = new THREE.PerspectiveCamera( 75, aspect, 0.1, 1000 )
        placeObjectCamera.position.set(0,150,10)
        this.cameras['Place Object'] = placeObjectCamera
    }
    ...
    updateCameraTarget() {
        ...
        else if(this.activeCameraName === "Parking Lot"){
            this.controls.target = new THREE.Vector3(0, -20, 0);
        }
        else if(this.activeCameraName === "Choose Object"){
